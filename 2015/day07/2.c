...
main(int const argc, char const * const * const argv)
{
	...
	struct htable * registers = hash_init(1024);
	...
	uint16_t interesting_signal = get_register(registers, interesting_register);

	assert(hash_free(registers, free));
	registers = hash_init(1024);
	assert(registers != NULL);

	set_register(registers, "b", interesting_signal);

	while (1) {
		for (size_t i = 0; i < n; i++) {
			struct Instruction const * const instr = &instructions[i];
			if (strcmp(instr->out_register, "b") == 0) {
				continue;
			}
			if (debug) {
				print_instruction(instr);
	...
	interesting_signal = get_register(registers, interesting_register);
	assert(hash_free(registers, free));
	printf("%d\n", interesting_signal);
	return 0;
}
