...
main(const int argc, const char * const * const argv)
{
	...
	assert(test0 != NULL);
	assert(strcmp(test0, "DDRRRD") == 0);
	free(test0);

	char * const test1 = __solve("kglvqrro");
	assert(test1 != NULL);
	assert(strcmp(test1, "DDUDRLRRUDRD") == 0);
	free(test1);

	char * const test2 = __solve("ulqzkmiv");
	assert(test2 != NULL);
	assert(strcmp(test2, "DRURDRUDDLLDLUURRDULRLDUUDDDRR") == 0);
	free(test2);
...
__solve(const char * const input)
{
	...
	while (q->first) {
		...
		if (current_map->x == 3 && current_map->y == 3) {
			char * const solution = calloc(MAX_MOVES + 1, sizeof(char));
			if (!solution) {
				printf("%s\n", strerror(errno));
				destroy_queue(q, __destroy_map);
			...
			strcat(solution, current_map->moves);
