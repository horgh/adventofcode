...
grid_new(int const);
static void
grid_destroy(int * * const, int const);
static int
grid_get(int * * const, int const, int const, int const);
...
int main(const int argc, const char * const * const argv)
{
	...
	for (int side_length = 1; ; side_length++) {
		...
		for (int square = 2; square <= num_squares; square++) {
			...
			int const up_right   = grid_get(grid, side_length, row_idx-1, col_idx+1);
			int const right      = grid_get(grid, side_length, row_idx,   col_idx+1);
			int const down_right = grid_get(grid, side_length, row_idx+1, col_idx+1);
			int const down       = grid_get(grid, side_length, row_idx+1, col_idx);
			int const down_left  = grid_get(grid, side_length, row_idx+1, col_idx-1);
			int const left       = grid_get(grid, side_length, row_idx,   col_idx-1);

			int const val = up_left+up+up_right+right+down_right+down+down_left+left;
			if (val > input) {
				printf("%d\n", val);
...
static int
grid_get(int * * const grid, int const side_length, int const row_idx,
		int const col_idx)
{
	if (row_idx < 0 || col_idx < 0 ||
			row_idx >= side_length || col_idx >= side_length) {
		return 0;
	}
