...
static struct scanner ** scanners_copy(struct scanner ** const, size_t const);
static void scanners_copy_shallow(
		struct scanner ** const, struct scanner ** const, size_t const);
...
scanners_copy(struct scanner ** const scanners, size_t const num_scanners)
{
	struct scanner ** const scanners_copy =
			calloc(num_scanners, sizeof(struct scanner *));
	if (!scanners_copy) {
		fprintf(stderr, "calloc(): %s\n", strerror(errno));
		return NULL;
	}
