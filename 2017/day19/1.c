...
static void
destroy_map(char * * const, size_t const);
static bool
travel(char * * const,
		size_t const,
		size_t const,
		...
		int const direction)
{
	...
	case DOWN:
		if (can_move(map, row_max, col_max, row, col, DOWN)) {
			return travel(map, row_max, col_max, row+1, col, visited, visited_size,
					DOWN);
		}
		if (can_move(map, row_max, col_max, row, col, RIGHT)) {
			if (travel(map, row_max, col_max, row, col+1, visited, visited_size,
						RIGHT)) {
				return true;
			}
		}
	...
	case RIGHT:
		if (can_move(map, row_max, col_max, row, col, RIGHT)) {
			return travel(map, row_max, col_max, row, col+1, visited, visited_size,
					RIGHT);
		}
		if (can_move(map, row_max, col_max, row, col, UP)) {
			if (travel(map, row_max, col_max, row-1, col, visited, visited_size,
						UP)) {
				return true;
			}
		}
