...
static bool maybe_attack(struct Square ** const, struct Creature const * const);
...
main(int const argc, char const * const * const argv)
{
	...
	while (1) {
		...
		for (y = 0; y < MAP_SZ; y++) {
			for (x = 0; x < MAP_SZ; x++) {
				...
				size_t n_targets = 0;
				find_targets(creatures, n_creatures, me, targets, &n_targets);
				if (n_targets == 0) {
					goto done;
				}

				take_turn(map, me, targets, n_targets);
				moved[me->id] = 1;
			}
		}
		rounds++;
	...
	print_answer(creatures, n_creatures, rounds);
	for (size_t i = 0; i < MAP_SZ; i++) {
		free(map[i]);
	}
	free(map);
...
static bool
maybe_attack(struct Square ** const map, struct Creature const * const me)
{
