...
static int count_units(struct Group ** const, size_t const, enum Type const);
...
static int simulate(struct Group ** const, size_t const, int const);
...
main(int const argc, char const * const * const argv)
{
	...
	int boost = 1;
	while (1) {
		printf("trying boost %d\n", boost);
		int const count = simulate(all_groups, n_all_groups, boost);
		if (count > 0) {
			break;
		}
		boost *= 2;
	}
	printf("found tentative boost %d\n", boost);
	while (1) {
		printf("trying boost %d\n", boost);
		int const count = simulate(all_groups, n_all_groups, boost);
		if (count > 0) {
			printf("wins with %d units\n", count);
		}
		if (count == 0) {
			break;
		}
...
static int
simulate(struct Group ** const orig_all_groups,
		...
		int const boost)
{
	...
	for (size_t i = 0; i < n_all_groups; i++) {
		all_groups[i] = calloc(1, sizeof(struct Group));
		assert(all_groups[i] != NULL);
		memcpy(all_groups[i], orig_all_groups[i], sizeof(struct Group));
	}

	for (size_t i = 0; i < n_all_groups; i++) {
		if (all_groups[i]->type != Immune) {
			continue;
		}
		all_groups[i]->ap += boost;
	}

	int round = 0;
	while (1) {
		if (count_units(all_groups, n_all_groups, Immune) == 0 ||
				count_units(all_groups, n_all_groups, Infection) == 0) {
			break;
		}
	...
	int const count = count_units(all_groups, n_all_groups, Immune);
...
static int
count_units(struct Group ** const all_groups,
		size_t const n_all_groups,
		enum Type const type)
{
	int count = 0;
	for (size_t i = 0; i < n_all_groups; i++) {
		if (all_groups[i]->type != type) {
			continue;
		}
		count += all_groups[i]->units;
	}
	return count;
}
