...
static void run(char const * const buf) {
	...
	int base = 0;
	while (1) {
		int64_t const x = run_program(program, n, &ip, &base, q, &done);
		if (done) {
			break;
		}
		int64_t const y = run_program(program, n, &ip, &base, q, &done);
		if (done) {
			break;
		}
		int64_t const type = run_program(program, n, &ip, &base, q, &done);
		if (done) {
			break;
		}
	...
	int count = 0;
	for (size_t y = 0; y < MAP_SZ; y++) {
		for (size_t x = 0; x < MAP_SZ; x++) {
			if (map[x][y].type == BLOCK) {
				count++;
			}
		}
	}
